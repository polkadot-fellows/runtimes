...
//! value-carrying flows need their assets and accounts set up. The upgrade does commit the SRS
//! chunk page hashes — [`crate::migrations::chunk_page_hashes::InitializeChunkPageHashes`] covers
//! every exponent this runtime uses: [`MembersFlexibleRingExponent`] and
//! [`LitePeopleRingExponent`] (`R2e9`), plus [`RecyclerRingExponent`] and
//! [`PaidUnloadTokenRingExponent`] (`R2e10`) for coinage. Everything below is still manual, in
//! order:
//!
//! 1. `ChunksManager::add_chunks` — upload the chunk pages themselves. This call is *permissionless
//!    and authorized*, not root: its validity comes from the page hashing to the value the
//!    migration committed, so anyone can supply the data.
//! 2. `MembersNotifier::subscribe` (Fellowship or root) — register Asset Hub Polkadot (para 1000)
...
//! 3. `Assets::force_create` (root) for [`StableAssetLocation`], unless HOLLAR is already
//!    registered locally. `CreateOrigin` is `EnsureNever` on this chain, so root is the only way.
//!    This is a prerequisite for step 5, which rejects an unknown asset.
//! 4. `Assets::force_set_metadata` (root) — set the HOLLAR asset metadata after creating it.
//! 5. Mint the coinage pallet account at least HOLLAR's minimum balance, then call
...
//! 6. Fund the pallet-derived accounts that pay out: [`GameAirdropSource`] (`pop/gads`) with the
...
//! 7. `Game::schedule_games` (Fellowship or root) — no meetup game exists until one is scheduled,
//!    so `pallet-game` and `pallet-score` stay dormant without this.
//! 8. `PeopleLite::set_attestation_allowance` (Fellowship or root) — admit a device-attestation
//!    provider.
//! 9. Run collators with offchain workers enabled: `PeopleAirdrops` schedules and executes its
