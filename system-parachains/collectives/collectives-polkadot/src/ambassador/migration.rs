...
pub(crate) mod add_accounts {
    ...
    use pallet_ranked_collective::{Config, Rank, MemberRecord};
    use pallet_ranked_collective::{IdToIndex, IndexToId, MemberCount, Members};
    #[cfg(feature = "try-runtime")]
    use sp_std::vec::Vec;
        ...
        <T as frame_system::Config>::AccountId: From<[u8; 32]>,
    {
        ...
        fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::TryRuntimeError> {
            let has_members = pallet_ranked_collective::Members::<T, AmbassadorCollectiveInstance>::iter().next().is_some();
        ...
        fn on_runtime_upgrade() -> Weight {
            ...
            for (desired_rank, account_id32) in Addresses::get() {
                ...
                IdToIndex::<T, I>::insert(desired_rank, who.clone(), count_at_rank.saturating_sub(1));
        ...
        fn post_upgrade(_state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
                ...
                MemberCount::<T, I>::get(0) == 13,
                "invalid members count at rank 0."
            );
            Ok(())
        }
    }
...
pub mod tests {
	...
	fn check_addresses() {
		...
		for (index, val) in ambassador_ss58.iter().enumerate() {
			...
			assert_eq!(
				addresses[index].0, ambassador_ss58[index].0,
				"ranks must be equal."
			);
