...
//! Placeholder weights for `pallet_revive` on `asset-hub-polkadot-runtime`.
//!
//! NOTE: These numbers are copied verbatim from the Asset Hub Kusama benchmark
//! output (generated 2026-04-09) so the runtime can compile while the benchmark
//! wiring is restored (see #840). They MUST be regenerated against
//! `asset-hub-polkadot-runtime` before release, e.g. by triggering
//! `/cmd bench --runtime asset-hub-polkadot --pallet pallet_revive` on the PR.

// Original execution context (asset-hub-kusama-runtime, BENCHMARK CLI VERSION 54.0.0):
// frame-omni-bencher
// v1
// benchmark
// pallet
