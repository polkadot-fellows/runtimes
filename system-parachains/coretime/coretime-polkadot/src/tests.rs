...
#[test]
fn bulk_revenue_goes_to_accumulation_account() {
	ExtBuilder::<Runtime>::default()
		...
		.execute_with(|| {
			...
			let broker_account: AccountId = BrokerPalletId::get().into_account_truncating();
			...
			let accumulation_balance_before = Balances::balance(&accumulation_account);
			...
			// Accumulation account gets exactly the revenue Alice paid.
			assert_eq!(
				Balances::balance(&accumulation_account),
				accumulation_balance_before + alice_paid
			);
		});
}
