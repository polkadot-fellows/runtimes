...
- Asset Hub Polkadot: `DustRemoval` resolves into the DAP staging account instead of burning. Dust now follows the same path as every other sink on Polkadot, which deactivates rather than destroys: `pallet-dap`'s `on_idle` drains staging into the buffer and deactivates the funds, so dust leaves active issuance without leaving total issuance. Both DAP accounts are already funded with the existential deposit, in genesis and on the live chain, which is also what keeps the dust-removal chain in [paritytech/polkadot-sdk#12130](https://github.com/paritytech/polkadot-sdk/issues/12130) from recursing: `on_idle` drains with `Preservation::Preserve`, so the staging account is never itself reapable ([#1283](https://github.com/polkadot-fellows/runtimes/issues/1283)).
