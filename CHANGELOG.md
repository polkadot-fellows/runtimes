...
- Asset Hub Polkadot: the `Assets` proxy can dispatch every `Psm` call, `AssetOwner` can create and remove a PSM and set its admins and external assets, and `AssetManager` can set its fees, debt limit, circuit breakers and ceiling weights ([#XXXX](https://github.com/polkadot-fellows/runtimes/pull/XXXX)).
