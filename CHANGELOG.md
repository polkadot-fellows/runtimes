...
- Bridge Hub, Coretime, People and Encointer Kusama: stop burning locally. `DustRemoval` now resolves into a `pallet-accumulate-and-forward` account which, at most hourly and once at least 0.1 KSM has gathered, teleports to Asset Hub and burns there via `BurnAsset`. KSM issuance is tracked on Asset Hub, so burning on a system chain lowered that chain's total while leaving Asset Hub's total and its XCM checking account untouched, overstating the real supply. Asset Hub Kusama needs no change: its barrier already grants sibling system parachains free execution and it already trusts them as KSM teleporters. **The accumulation account on each of the four chains must be funded with the existential deposit before these runtimes go live**, otherwise `can_deposit` rejects sub-existential-deposit dust and it is burned as before; the runtimes log an error and `try-runtime` fails if it is missing ([#1283](https://github.com/polkadot-fellows/runtimes/issues/1283)).
