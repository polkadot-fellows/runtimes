...
- Bridge Hub, Coretime, People and Encointer Kusama: the sinks that used to burn locally now accumulate and are teleported to Asset Hub to be burned there, with Coretime's `py/ctbrn` holding account retired so `pallet-accumulate-and-forward` is the only path ([#1287](https://github.com/polkadot-fellows/runtimes/pull/1287)).
