...
- Collectives Polkadot: the `NonTransfer` proxy no longer admits salary `payout_other`, which pays the member's salary to any account, and is no longer a superset of `Fellowship` or `Ambassador` ([#XXXX](https://github.com/polkadot-fellows/runtimes/pull/XXXX)).
