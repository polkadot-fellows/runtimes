...
- People Polkadot: pay transaction fees, XCM execution fees and XCM *delivery* fees in any asset, priced by whichever of an `pallet-asset-conversion` liquidity pool and a `pallet-asset-rate` rate asks the payer for less.
  - **Pools.** Anyone can open and seed a DOT pool for a registered asset and arbitrage its price, so enabling an asset for fees no longer needs a governance-set rate — the market prices it, the way Polkadot Asset Hub already works. Fees stay quoted in DOT throughout; the pool sells just enough of the offered asset to cover them, and the DOT is what lands in the fee account.
  - **Cheaper of the two.** `pallet-asset-rate` is kept, and the two oracles compete rather than one taking precedence. This is deliberate: anyone may open a pool for any registered asset, and a pool thin enough that a single fee moves its price can quote far above the registered rate — slippage, unlike a mispricing, is not something arbitrage repairs. Taking the cheaper quote makes that unprofitable to attempt while still letting a real pool beat a stale rate. Assets with only a rate keep working unchanged, so HOLLAR needs no pool and no migration.
  - Before this change the three fee kinds were inconsistent: transaction fees already accepted any rated asset, XCM execution fees were hardcoded to HOLLAR, and XCM delivery fees could only be paid in DOT. All three now follow the same rule.
  - Transaction fees move from `pallet-asset-tx-payment` to `pallet-asset-conversion-tx-payment`, whose `ChargeAssetTxPayment` encodes identically (a compact tip and an optional asset `Location`) and keeps the same extension identifier, so signers are unaffected and `transaction_version` does not move. The pallet takes a new index (19), since its `AssetTxFeePaid` event has a different shape from the old pallet's.
  - `XcmPaymentApi::query_acceptable_payment_assets` lists DOT, every pooled asset and every rated asset, and the new `AssetConversionApi` exposes pool prices and reserves.
  - `IsReserve` now also accepts assets *native to Asset Hub* — the ones its trust backed `Assets` and pool instances issue — when sent from Asset Hub, on top of HOLLAR from Hydration, so an asset registered there can be used here. It is deliberately restricted to Asset Hub's own assets rather than everything Asset Hub custodies: trusting a chain as reserve for an asset it did not issue gives that asset two reserves, and `ReserveAssetDeposited` mints locally, so the second reserve could credit this chain with holdings the real reserve is not backing — including DOT, which `FungibleTransactor` mints with no checking account. Reserve trust is also paired with a narrow registry: `pallet-assets` has `CreateOrigin = EnsureNever`, so an incoming asset is only credited if root already registered it locally.
