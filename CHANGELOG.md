...
- Kusama relay: `DustRemoval` resolves into a `pallet-accumulate-and-forward` account which, at most hourly and once at least 0.01 KSM has gathered, teleports to Asset Hub and burns there via `BurnAsset`. KSM issuance is tracked on Asset Hub, so burning on the relay lowered its total while leaving Asset Hub's total and its XCM checking account untouched, overstating the real supply. Asset Hub Kusama needs no change: it already trusts the relay as a KSM teleporter and grants it free execution. **The accumulation account must be funded with the existential deposit before this runtime goes live**, otherwise `can_deposit` rejects sub-existential-deposit dust and it is burned as before; the runtime logs an error and `try-runtime` fails if it is missing ([#1283](https://github.com/polkadot-fellows/runtimes/issues/1283)).
