...
- Asset Hub Polkadot: make the `WhitelistedCaller` origin usable over XCM as a `Plurality` voice (`BodyId::Index(2)`) under the Asset Hub location; People Polkadot: recognize that voice through the new `RootOrWhitelistedCaller` origin.
