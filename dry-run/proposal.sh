...
#
# Accepts either a preimage hash (looked up on-chain) or raw call data.
# Automatically detects relay vs parachain and picks the correct scheduler
# block number. For XCM calls that target another chain, pass --receiver
# to also produce a block on the receiving chain and print its events.
#
# Prerequisites:
...
cd "$SCRIPT_DIR" && exec node --input-type=module - "$@" << 'SCRIPT'
