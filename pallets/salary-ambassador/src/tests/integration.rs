...
use pallet_ranked_collective_ambassador::{EnsureRanked, Geometric};
...
fn swap_simple_works() {
	new_test_ext().execute_with(|| {
		for i in 0u16..9 {
			...
			assert_ok!(Club::add_member(RuntimeOrigin::signed(acc), acc));
...
fn swap_exhaustive_works() {
	new_test_ext().execute_with(|| {
		...
		let root_swap = hypothetically!({
			assert_ok!(Club::add_member(RuntimeOrigin::signed(0), 0));
