...
parameter_types! {
	...
	pub const InductionDeposit: u64 = 25;
}
...
fn add_remove_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(Club::add_member(RuntimeOrigin::signed(2), 2));
		assert_eq!(member_count(0), 2);

		assert_ok!(Club::add_member(RuntimeOrigin::signed(3), 3));
...
fn voting_works() {
	ExtBuilder::default().build_and_execute(|| {
		assert_ok!(Club::add_member(RuntimeOrigin::signed(1), 0));
		assert_ok!(Club::add_member(RuntimeOrigin::signed(1), 1));
		assert_ok!(Club::promote_member(RuntimeOrigin::root(), 1));
		assert_ok!(Club::add_member(RuntimeOrigin::signed(2), 2));
		assert_ok!(Club::promote_member(RuntimeOrigin::root(), 2));
...
fn do_add_member_to_rank_works() {
	ExtBuilder::default().build_and_execute(|| {
		let max_rank = 9u16;
		assert_ok!(Club::do_add_member_to_rank(31, max_rank / 2, true));
		assert_ok!(Club::do_add_member_to_rank(40, max_rank, true));
		for i in 0..=max_rank {
			if i <= max_rank / 2 {
...
#[test]
fn deposit_reserved_on_induction() {
	ExtBuilder::default().build_and_execute(|| {
		let deposit = <Test as Config>::InductionDeposit::get();
		let balance_ad = Balances::free_balance(&1) - deposit;
		assert_ok!(Club::add_member(RuntimeOrigin::signed(1), 1));
		assert_eq!(Balances::reserved_balance(&1), deposit);
		assert_eq!(Balances::free_balance(&1), balance_ad);
	});
}
