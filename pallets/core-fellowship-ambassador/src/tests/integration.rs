...
use pallet_ranked_collective_ambassador::{EnsureRanked, Geometric, Rank};
use sp_core::Get;
use sp_runtime::{
	bounded_vec,
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	ext.execute_with(|| {
		assert_ok!(Club::add_member(signed(100), 100));
...
fn import_simple_works() {
	new_test_ext().execute_with(|| {
		for i in 0u16..9 {
			...
			assert_ok!(Club::add_member(signed(acc), acc));
...
fn swap_exhaustive_works() {
	new_test_ext().execute_with(|| {
		let root_add = hypothetically!({
			assert_ok!(Club::add_member(signed(1), 1));
			promote_n_times(1, 4);
		...
		let root_swap = hypothetically!({
			assert_ok!(Club::add_member(signed(1), 0));
			promote_n_times(0, 4);
...
fn swap_bad_noops() {
	new_test_ext().execute_with(|| {
		assert_ok!(Club::add_member(signed(0), 0));
		promote_n_times(0, 0);
		assert_ok!(CoreFellowship::import(signed(0)));
		assert_ok!(Club::add_member(signed(1), 1));
		promote_n_times(1, 1);
		...
		assert_noop!(
			...
			pallet_ranked_collective_ambassador::Error::<Test>::AlreadyMember
		);
		// Swapping for the same member is a noop:
		assert_noop!(
			Club::exchange_member(RuntimeOrigin::root(), 0, 0),
			pallet_ranked_collective_ambassador::Error::<Test>::SameMember
		);
	});
}