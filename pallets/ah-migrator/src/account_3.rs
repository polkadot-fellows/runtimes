...
impl<T: Config> Pallet<T> {
	...
	pub fn do_receive_account(
		...
		//[2m2025-11-03T17:59:11.249058Z[0m [33m WARN[0m [2mruntime::ah-migrator[0m[2m:[0m
		//[2m2025-11-03T17:59:11.249058Z[0m Dusting Alert! Account
		//[2m2025-11-03T17:59:11.249058Z[0m 5DoXzayUMdzhERn5Zz1jvaV4J4eDjGfzxbcx7eSd5FLTED9d has
		//[2m2025-11-03T17:59:11.249058Z[0m less total balance 10838485409 than its migrated total
		//[2m2025-11-03T17:59:11.249058Z[0m balance 10839485409
		// 10_838_485_409 10_839_485_409
		...
		fn post_check(rc_pre_payload: Self::RcPrePayload, ah_pre_payload: Self::AhPrePayload) {
			...
			for as_para in paras_on_rc {
				...
				assert!(
					...
					"Sovereign parachain account {:?} unexpectedly contains Freezes. Freezes are not migrated and should not exist for parachain accounts. Please review account state.",
					as_para
				);
			...
			let test_account: AccountId32 =
				"1sCHarneLcQgVUCjzoKsaEpcybABfJVmSbQD6UgGVpT57Xw".parse().unwrap();
			...
			for (who, summary) in account_summaries {
				...
				if who == test_account {
					log::info!(
						"Test account: {:?} \n total balance: {:?}\n free: {:?}\n reserved: {:?}, holds: {:?}, freezes: {:?}, locks: {:?}",
						who.to_ss58check(),
						<T as Config>::Currency::total_balance(&who),
						<T as Config>::Currency::balance(&who),
						<T as Config>::Currency::reserved_balance(&who),
						pallet_balances::Holds::<T>::get(&who).iter().map(|h| (h.id.encode(), h.amount)).collect::<Vec<(Vec<u8>, u128)>>(),
						pallet_balances::Freezes::<T>::get(&who).iter().map(|f| (f.id.encode(), f.amount)).collect::<Vec<(Vec<u8>, u128)>>(),
						pallet_balances::Locks::<T>::get(&who).into_inner().iter().map(|l| (l.id.encode(), l.amount, l.reasons as u8)).collect::<Vec<(Vec<u8>, u128, u8)>>(),
					);
				}
			...
			if !balance_diff_stats.is_empty() {
				...
				log::error!(
					target: LOG_TARGET,
					"Total account count: {:?}, Total missing free: {:?}, total excess balance: {:?}",
			...
			if !reserved_diff_stats.is_empty() {
				...
				log::error!(
					target: LOG_TARGET,
					"Total account count: {:?}, Total missing reserved: {:?}, total excess reserved: {:?}",
