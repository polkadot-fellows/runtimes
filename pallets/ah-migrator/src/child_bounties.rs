...
{
	pub fn do_receive_child_bounties_messages(messages: Vec<PortableChildBountiesMessage>) -> Result<(), Error<T>> {
	...
	fn do_receive_child_bounties_message(message: PortableChildBountiesMessage) -> Result<(), Error<T>> {
		...
		match message {
			...
			ChildBounty { parent_id, child_id, child_bounty } => {
				let child_bounty: pallet_child_bounties::ChildBounty<AccountId32, pallet_treasury::BalanceOf<T>, pallet_treasury::BlockNumberFor<T>> = child_bounty.into();
				pallet_child_bounties::ChildBounties::<T>::insert(parent_id, child_id, child_bounty);
			}
			ChildBountyDescriptionsV1 { parent_id, child_id, description } => {
				...
				let description = description.into_iter().take(<T as pallet_bounties::Config>::MaximumReasonLength::get() as usize).collect::<Vec<_>>();
				let description = BoundedVec::try_from(description).defensive().unwrap_or_default();

				pallet_child_bounties::ChildBountyDescriptionsV1::<T>::insert(parent_id, child_id, description);
			}
			V0ToV1ChildBountyIds { v0_child_id, parent_id, v1_child_id } => {
				pallet_child_bounties::V0ToV1ChildBountyIds::<T>::insert(v0_child_id, (parent_id, v1_child_id));
			}
