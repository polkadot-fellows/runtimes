...
pub mod tests {
	...
	impl<T: Config> crate::types::AhMigrationCheck for AccountsMigrationChecker<T> {
		...
		fn post_check(rc_pre_payload: Self::RcPrePayload, ah_pre_payload: Self::AhPrePayload) {
			...
			let mut diff_stats = BTreeMap::<T::AccountId, (u128, u128, u128, u128)>::new();
			...
			for (who, summary) in account_summaries {
				...
				let ah_migrated_free_balance =
					ah_free_post.checked_sub(ah_free_before).unwrap_or_else(|| {
						log::error!(target: LOG_TARGET, "test1");
						...
						diff_stats
							...
							.or_insert((missing_free, 0, 0, 0));
						0
					});

				let ah_migrated_free_balance = ah_migrated_free_balance
					.saturating_add(ah_reserved_post)
					.saturating_sub(ah_reserved_before);

				let ah_free_balance_diff = summary
					.migrated_free
					.checked_sub(ah_migrated_free_balance)
					.unwrap_or_else(|| {
						log::error!(target: LOG_TARGET, "test2");
						let extra_free =
							ah_migrated_free_balance.saturating_add(summary.migrated_free);
						diff_stats
							.entry(who.clone())
							.and_modify(|entry| entry.1 = extra_free)
							.or_insert((0, extra_free, 0, 0));
						0
					});
				...
				if !diff_stats.contains_key(&who) && ah_free_balance_diff >= ah_ed {
					log::error!(target: LOG_TARGET, "test3");
					diff_stats
						.entry(who.clone())
						.and_modify(|entry| entry.1 = ah_free_balance_diff)
						.or_insert((0, ah_free_balance_diff, 0, 0));
				}

				if let Some(ah_migrated_reserved_balance) =
					ah_reserved_post.checked_sub(ah_reserved_before)
				{
					// There are several `unreserve` operations on AH after migration (e.g.,
					// unreserve deposits for multisigs because they are not migrated to AH,
					// adjust deposits for preimages, ...). Therefore, we just check that the
					// change in reserved balance on AH after migration is less than the
					// migrated reserved balance from RC.
					if ah_migrated_reserved_balance <= summary.migrated_reserved {
						log::error!(target: LOG_TARGET, "test5");
						let extra_reserved =
							ah_migrated_reserved_balance.saturating_sub(summary.migrated_reserved);
						diff_stats
							.entry(who.clone())
							.and_modify(|entry| entry.3 = extra_reserved)
							.or_insert((0, 0, 0, extra_reserved));
					}
				} else {
					log::error!(target: LOG_TARGET, "test4");
					let missing_reserved = ah_reserved_before
						.saturating_add(summary.migrated_reserved)
						.saturating_sub(ah_reserved_post);
					diff_stats
						.entry(who.clone())
						.and_modify(|entry| entry.2 = missing_reserved)
						.or_insert((0, 0, missing_reserved, 0));
				}
			...
			if !diff_stats.is_empty() {
				let mut total_extra_free = 0;
				let mut total_missing_free = 0;
				let mut total_extra_reserved = 0;
				let mut total_missing_reserved = 0;

				for (account, (extra_free, missing_free, extra_reserved, missing_reserved)) in
					&diff_stats
				{
					total_extra_free += extra_free;
					total_missing_free += missing_free;
					total_extra_reserved += extra_reserved;
					total_missing_reserved += missing_reserved;
					log::error!(
						target: LOG_TARGET,
						"Account {:?}: extra_free: {:?}, missing_free: {:?}, extra_reserved: {:?}, missing_reserved: {:?}",
				...
				log::error!(
					target: LOG_TARGET,
					"Total account count: {:?}, Total missing free: {:?}, total extra free: {:?}, total missing reserved: {:?}, total extra reserved: {:?}",
					diff_stats.len(),
					total_missing_free,
					total_extra_free,
					total_missing_reserved,
					total_extra_reserved
				);
			}
		}
	}
