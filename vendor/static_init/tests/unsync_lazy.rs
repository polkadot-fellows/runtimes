...
fn local_lazy() {
    ...
    {
        UnSyncLazy::new(|| A(&mut drop_count));
    }
    assert_eq!(drop_count, 0);
    {
        let v = UnSyncLazy::new(|| A(&mut drop_count));
        LazyAccess::init(&v);
    }
...
fn local_lazy_mut() {
    ...
    assert!(UnSyncLazy::try_get_mut(&mut v).is_err());
    ...
    UnSyncLazy::try_get_mut(&mut v).unwrap().push(4);
