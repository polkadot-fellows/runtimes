...
pub fn penpal_set_foreign_asset_reserves_on_asset_hub(
	...
	let call = <AssetHubPolkadot as Chain>::RuntimeCall::ForeignAssets(pallet_assets::Call::<
	...
	let asset_hub_location = PenpalA::sibling_location_of(AssetHubPolkadot::para_id()).into();
	let xcm =
...
fn bidirectional_teleport_foreign_asset_between_penpal_and_asset_hub() {
	...
	let dest = PenpalA::sibling_location_of(AssetHubPolkadot::para_id());
	let assets: Assets =
	...
	let expected_origin = AssetHubPolkadot::sibling_location_of(PenpalA::para_id());
	...
	AssetHubPolkadot::execute_with(|| {
		...
		assert!(matches!(
			<AssetHubPolkadot as AssetHubPolkadotPallet>::PolkadotXcm::execute(
				<AssetHubPolkadot as Chain>::RuntimeOrigin::signed(receiver.clone()),
				bx!(xcm::VersionedXcm::from(xcm.into())),
				Weight::MAX,
			),
