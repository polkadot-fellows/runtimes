...
use crate::*;
use asset_hub_polkadot_runtime::RuntimeOrigin as AssetHubRuntimeOrigin;
...
fn test_set_asset_claimer_within_a_chain() {
	...
	let alice_balance_before =
		<AssetHubPolkadot as Chain>::account_data_of(alice_account.clone()).free;
	AssetHubPolkadot::fund_accounts(vec![(alice_account.clone(), trap_amount * 2)]);
	...
	let bob_balance_before = <AssetHubPolkadot as Chain>::account_data_of(bob_account.clone()).free;
	...
	let bob_balance_after = <AssetHubPolkadot as Chain>::account_data_of(bob_account.clone()).free;
	assert_eq!(bob_balance_after - bob_balance_before, trap_amount);
}

fn account_and_location(account: &str) -> (AccountId32, Location) {
	let account_id = AssetHubPolkadot::account_id_of(account);
	let account_clone = account_id.clone();
	let location: Location =
		[Junction::AccountId32 { network: Some(NetworkId::Polkadot), id: account_id.into() }]
			.into();
...
fn test_set_asset_claimer_between_the_chains() {
	...
	let alice_bh_sibling = Location::new(
		...
		[
			...
			Junction::AccountId32 { network: Some(NetworkId::Polkadot), id: alice.clone().into() },
		],
	);

	let bob = BridgeHubPolkadot::account_id_of(BOB);
	let trap_amount = 16_000_000_000_000u128;
	BridgeHubPolkadot::fund_accounts(vec![(bob.clone(), trap_amount * 2)]);
	...
	let balance = <BridgeHubPolkadot as Chain>::account_data_of(alice_bh_acc.clone()).free;
	assert_eq!(balance, 0);
	...
	let xcm_on_bh = Xcm::<()>::builder_unsafe()
		...
		.build();
	let bh_on_ah = AssetHubPolkadot::sibling_location_of(BridgeHubPolkadot::para_id()).into();
	AssetHubPolkadot::execute_with(|| {
	...
	let alice_bh_acc = LocationToAccountId::convert_location(&alice_bh_sibling).unwrap();
	let balance = <BridgeHubPolkadot as Chain>::account_data_of(alice_bh_acc).free;
