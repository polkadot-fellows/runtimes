...
fn para_to_relay_receiver_assertions(t: ParaToRelayTest) {
	...
	Polkadot::assert_ump_queue_processed(true, Some(PenpalA::para_id()), None);
	assert_expected_events!(
		Polkadot,
		vec![
...
fn para_to_para_relay_hop_assertions(t: ParaToParaThroughRelayTest) {
	...
	assert_expected_events!(
		...
		vec![
			...
			RuntimeEvent::Balances(
				pallet_balances::Event::Withdraw { who, amount } | pallet_balances::Event::Withdraw { who, amount }
			) => {
			...
			RuntimeEvent::Balances(
				pallet_balances::Event::Deposit { who, .. } | pallet_balances::Event::Deposit { who, .. }
			) => {
...
/// Reserve Transfers of DOT from Parachain to Relay should work
#[test]
fn reserve_transfer_dot_from_para_to_relay() {
	// Init values for Parachain
	...
	test.set_assertion::<Polkadot>(para_to_relay_receiver_assertions);
...
/// Reserve Transfers of DOT from Asset Hub to Parachain should work
#[test]
fn reserve_transfer_dot_from_asset_hub_to_para() {
	// Init values for Asset Hub
...
/// Reserve Transfers of DOT from Parachain to Asset Hub should work
#[test]
fn reserve_transfer_dot_from_para_to_asset_hub() {
	// Init values for Parachain
...
/// Reserve Transfers of a local asset and DOT from Asset Hub to Parachain should work
#[test]
fn reserve_transfer_multiple_assets_from_asset_hub_to_para() {
	// Init values for Asset Hub
	let destination = AssetHubPolkadot::sibling_location_of(PenpalB::para_id());
	let sov_penpal_on_ahp = AssetHubPolkadot::sovereign_account_id_of(destination.clone());
	let sender = AssetHubPolkadotSender::get();
	...
	AssetHubPolkadot::fund_accounts(vec![(sov_penpal_on_ahp, ASSET_HUB_POLKADOT_ED)]);
...
/// Reserve Transfers of a random asset and DOT asset from Parachain to Asset Hub should work
#[test]
fn reserve_transfer_multiple_assets_from_para_to_asset_hub() {
	...
	let sender = PenpalBSender::get();
	let fee_amount_to_send = ASSET_HUB_POLKADOT_ED * 10000;
	let asset_amount_to_send = ASSET_HUB_POLKADOT_ED * 10000;
	let penpal_asset_owner = PenpalAssetOwner::get();
	let penpal_asset_owner_signer = <PenpalB as Chain>::RuntimeOrigin::signed(penpal_asset_owner);
	let asset_location_on_penpal = PenpalLocalReservableFromAssetHub::get();
	let system_asset_location_on_penpal = DotLocation::get();
	let assets: Assets = vec![
		(Parent, fee_amount_to_send).into(),
	...
	let sov_penpal_on_ahp =
		AssetHubPolkadot::sovereign_account_id_of(penpal_location_as_seen_by_ahp);
	let ah_asset_owner = AssetHubPolkadotAssetOwner::get();
	let ah_asset_owner_signer = <AssetHubPolkadot as Chain>::RuntimeOrigin::signed(ah_asset_owner);

	// Fund SA-of-Penpal-on-AHP to be able to pay for the fees.
	AssetHubPolkadot::fund_accounts(vec![(
		sov_penpal_on_ahp.clone(),
		ASSET_HUB_POLKADOT_ED * 10000000,
	)]);
	// Fund SA-of-Penpal-on-AHP to be able to pay for the sent amount.
	AssetHubPolkadot::mint_asset(
	...
	test.set_assertion::<AssetHubPolkadot>(para_to_system_para_assets_receiver_assertions);
	test.set_dispatchable::<PenpalB>(para_to_system_para_reserve_transfer_assets);
	test.assert();
...
/// Reserve Transfers of DOT from Parachain to Parachain (through Relay reserve) should work
#[test]
	...
	for asset in t.args.assets.into_inner().into_iter() {
		...
		let relative_id = match expected_id {
			...
			_ => {
				...
				id.push_front_interior(Parachain(AssetHubPolkadot::para_id().into())).unwrap();
...
fn reserve_transfer_usdt_from_asset_hub_to_para() {
	...
	let penpal_sov_account = AssetHubPolkadot::sovereign_account_id_of(penpal_location.clone());

	// Create SA-of-Penpal-on-AHP with ED.
	AssetHubPolkadot::fund_accounts(vec![(penpal_sov_account.clone(), ASSET_HUB_POLKADOT_ED)]);
	...
	let sender_initial_balance = AssetHubPolkadot::execute_with(|| {
		type Assets = <AssetHubPolkadot as AssetHubPolkadotPallet>::Assets;
		<Assets as Inspect<_>>::balance(usdt_id, &sender)
	});
	let sender_initial_native_balance = AssetHubPolkadot::execute_with(|| {
	...
	test.set_assertion::<AssetHubPolkadot>(usdt_sender_assertions);
	test.set_assertion::<PenpalB>(system_para_to_penpal_receiver_assertions);
	test.set_dispatchable::<AssetHubPolkadot>(system_para_to_para_reserve_transfer_assets);
	test.assert();

	let sender_after_balance = AssetHubPolkadot::execute_with(|| {
		type Assets = <AssetHubPolkadot as AssetHubPolkadotPallet>::Assets;
		<Assets as Inspect<_>>::balance(usdt_id, &sender)
	});
	let sender_after_native_balance = AssetHubPolkadot::execute_with(|| {
		type Balances = <AssetHubPolkadot as AssetHubPolkadotPallet>::Balances;
		Balances::free_balance(&sender)
	});
...
fn reserve_transfer_usdt_from_para_to_para_through_asset_hub() {
	...
	let sender = PenpalASender::get();
	let asset_amount_to_send: Balance = POLKADOT_ED * 10000;
