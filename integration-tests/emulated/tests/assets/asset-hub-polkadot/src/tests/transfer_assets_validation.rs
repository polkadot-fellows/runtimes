...
/// Test that `transfer_assets` fails when doing reserve transfer of DOT from PenpalA to Polkadot.
/// This fails because PenpalA's IsReserve config considers Polkadot as the reserve for DOT,
/// so transfer_assets automatically chooses reserve transfer, which we block.
#[test]
fn transfer_assets_dot_reserve_transfer_para_to_relay_fails() {
	let destination = PenpalA::parent_location();
	let beneficiary: Location =
		AccountId32Junction { network: None, id: PolkadotReceiver::get().into() }.into();
	let amount_to_send: Balance = POLKADOT_ED * 1000;
	let assets: Assets = (Parent, amount_to_send).into();

	// Mint DOT on PenpalA for testing.
	...
	PenpalA::execute_with(|| {
		...
		// This should fail because DOT reserve transfer is blocked.
		assert_err!(
			result,
...
/// Test that `transfer_assets` fails when doing reserve transfer of DOT from Polkadot to PenpalA
/// This fails because Polkadot's configuration would make this a reserve transfer, which we block.
#[test]
fn transfer_assets_dot_reserve_transfer_relay_to_para_fails() {
	let destination = Polkadot::child_location_of(PenpalA::para_id());
	let beneficiary: Location =
	...
	Polkadot::execute_with(|| {
		let result = <Polkadot as PolkadotPallet>::XcmPallet::transfer_assets(
...
/// Test that `transfer_assets` fails when doing reserve transfer of DOT from PenpalA to PenpalB
#[test]
fn transfer_assets_dot_reserve_transfer_para_to_para_fails() {
...
/// Test that `transfer_assets` fails when DOT is used as fee asset in reserve transfer
#[test]
fn transfer_assets_dot_as_fee_in_reserve_transfer_fails() {
	let destination = PenpalA::sibling_location_of(PenpalB::para_id());
	let beneficiary: Location =
		AccountId32Junction { network: None, id: PenpalBReceiver::get().into() }.into();
	let asset_amount: Balance = 1_000_000_000_000; // A million USDT.
	let fee_amount: Balance = POLKADOT_ED * 100;
	...
	// Transfer foreign asset, pay fees with DOT.
	let assets: Assets = vec![
		(foreign_asset_location, asset_amount).into(),
		(Parent, fee_amount).into(), // DOT as fee.
	]
	.into();
	let fee_asset_item = 1; // DOT is the fee asset.
	...
	PenpalA::execute_with(|| {
		...
		// This should fail because DOT fee would be reserve transferred.
		assert_err!(
			result,
			DispatchError::Module(sp_runtime::ModuleError {
				index: 31,
...
/// Test that `transfer_assets` works when neither asset nor fee is DOT.
#[test]
fn transfer_assets_non_native_assets_work() {
	...
	// Mint both USDT and DOT on PenpalA, one for sending, the other for paying delivery fees.
	PenpalA::mint_foreign_asset(
		<PenpalA as Chain>::RuntimeOrigin::signed(PenpalAssetOwner::get()),
		DotLocation::get(),
		PenpalASender::get(),
	...
	PenpalA::execute_with(|| {
		...
		// This should succeed because neither asset is DOT.
		assert_ok!(result);
	});
}
