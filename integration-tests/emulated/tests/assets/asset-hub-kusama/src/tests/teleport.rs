...
pub fn do_bidirectional_teleport_foreign_assets_between_para_and_asset_hub_using_xt(
	...
	let asset_location_on_penpal_v4: xcm::v4::Location = asset_location_on_penpal.try_into().unwrap();
	...
	let penpal_as_seen_by_ah = AssetHubKusama::sibling_location_of(PenpalA::para_id());
	let foreign_asset_at_asset_hub_kusama_latest: Location = foreign_asset_at_asset_hub_kusama.clone().try_into().unwrap();
