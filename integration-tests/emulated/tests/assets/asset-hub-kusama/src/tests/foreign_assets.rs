...
pub fn penpal_set_foreign_asset_reserves_on_asset_hub(
	...
	let call = <AssetHubKusama as Chain>::RuntimeCall::ForeignAssets(pallet_assets::Call::<
...
fn bidirectional_teleport_foreign_asset_between_penpal_and_asset_hub() {
	...
	let dest = PenpalA::sibling_location_of(AssetHubKusama::para_id());
	let assets: Assets =
	...
	let expected_origin = AssetHubKusama::sibling_location_of(PenpalA::para_id());
	...
	AssetHubKusama::execute_with(|| {
		...
		assert!(matches!(
			<AssetHubKusama as AssetHubKusamaPallet>::PolkadotXcm::execute(
				<AssetHubKusama as Chain>::RuntimeOrigin::signed(receiver.clone()),
				bx!(xcm::VersionedXcm::from(xcm.into())),
				Weight::MAX,
			),
...
fn verify_foreign_asset_origin_checks() {
	...
	let reserves_data = ForeignAssetReserveData {
		reserve: AssetHubKusama::sibling_location_of(PenpalA::para_id()),
		teleportable: true,
	};
	// Set asset reserves using signed `owner` account.
	let origin = <AssetHubKusama as Chain>::RuntimeOrigin::signed(penpal_sovereign);
	AssetHubKusama::execute_with(|| {
		type RuntimeEvent = <AssetHubKusama as Chain>::RuntimeEvent;
		<AssetHubKusama as AssetHubKusamaPallet>::ForeignAssets::set_reserves(
			origin,
	...
	let origin = <AssetHubKusama as Chain>::RuntimeOrigin::signed(sender.clone());
	AssetHubKusama::execute_with(|| {
		assert!(<AssetHubKusama as AssetHubKusamaPallet>::ForeignAssets::set_reserves(
			origin,
			foreign_asset_location_on_ah.clone(),
