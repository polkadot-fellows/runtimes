...
fn safe_mode_blocks_non_whitelisted_calls() {
	...
	let bob = AssetHubKusama::account_id_of(BOB);

	AssetHubKusama::execute_with(|| {
		let signed: RuntimeOrigin = <AssetHubKusama as Chain>::RuntimeOrigin::signed(alice.clone());
		let root: RuntimeOrigin = <AssetHubKusama as Chain>::RuntimeOrigin::root();
		...
		assert_err!(
			...
			frame_system::Error::<asset_hub_kusama_runtime::Runtime>::CallFiltered,
		);
...
fn tx_pause_callable_during_safe_mode() {
	...
	AssetHubKusama::execute_with(|| {
		let signed: RuntimeOrigin = <AssetHubKusama as Chain>::RuntimeOrigin::signed(alice);
		let root: RuntimeOrigin = <AssetHubKusama as Chain>::RuntimeOrigin::root();
