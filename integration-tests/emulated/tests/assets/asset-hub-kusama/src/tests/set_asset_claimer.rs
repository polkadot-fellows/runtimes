...
use crate::*;
use asset_hub_kusama_runtime::RuntimeOrigin as AssetHubRuntimeOrigin;
use bridge_hub_kusama_runtime::{
	xcm_config::LocationToAccountId, RuntimeOrigin as BridgeHubRuntimeOrigin,
};
...
fn test_set_asset_claimer_within_a_chain() {
	...
	let alice_balance_before =
		<AssetHubKusama as Chain>::account_data_of(alice_account.clone()).free;
	AssetHubKusama::fund_accounts(vec![(alice_account.clone(), trap_amount * 2)]);
	...
	let balance_after_trap = <AssetHubKusama as Chain>::account_data_of(alice_account.clone()).free;
	assert_eq!(alice_balance_after - balance_after_trap, trap_amount);

	let bob_balance_before = <AssetHubKusama as Chain>::account_data_of(bob_account.clone()).free;
	...
	let bob_balance_after = <AssetHubKusama as Chain>::account_data_of(bob_account.clone()).free;
	assert_eq!(bob_balance_after - bob_balance_before, trap_amount);
}

fn account_and_location(account: &str) -> (AccountId32, Location) {
	let account_id = AssetHubKusama::account_id_of(account);
	let account_clone = account_id.clone();
	let location: Location =
		[Junction::AccountId32 { network: Some(NetworkId::Kusama), id: account_id.into() }].into();
	(account_clone, location)
}

#[test]
fn test_set_asset_claimer_between_the_chains() {
	let alice = AssetHubKusama::account_id_of(ALICE);
	let alice_bh_sibling = Location::new(
		1,
		[
			Parachain(AssetHubKusama::para_id().into()),
			Junction::AccountId32 { network: Some(NetworkId::Kusama), id: alice.clone().into() },
		],
	);

	let bob = BridgeHubKusama::account_id_of(BOB);
	let trap_amount = 16_000_000_000_000u128;
	BridgeHubKusama::fund_accounts(vec![(bob.clone(), trap_amount * 2)]);
	...
	let balance = <BridgeHubKusama as Chain>::account_data_of(alice_bh_acc.clone()).free;
	assert_eq!(balance, 0);
	...
	let xcm_on_bh = Xcm::<()>::builder_unsafe()
		...
		.build();
	let bh_on_ah = AssetHubKusama::sibling_location_of(BridgeHubKusama::para_id()).into();
	AssetHubKusama::execute_with(|| {
	...
	let alice_bh_acc = LocationToAccountId::convert_location(&alice_bh_sibling).unwrap();
	let balance = <BridgeHubKusama as Chain>::account_data_of(alice_bh_acc).free;
