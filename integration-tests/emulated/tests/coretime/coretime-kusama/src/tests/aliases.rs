...
use crate::*;
use coretime_kusama_runtime::xcm_config::XcmConfig;
...
fn account_on_sibling_syschain_aliases_into_same_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubKusama, CoretimeKusama, TELEPORT_FEES, ALLOWED),
			// between BH and Coretime: allowed
			(BridgeHubKusama, CoretimeKusama, TELEPORT_FEES, ALLOWED),
			// between People and Coretime: allowed
			(PeopleKusama, CoretimeKusama, TELEPORT_FEES, ALLOWED),
			// between Penpal and Coretime: denied
			(PenpalA, CoretimeKusama, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
		target,
		fees
...
fn account_on_sibling_chain_cannot_alias_into_different_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubKusama, CoretimeKusama, TELEPORT_FEES, DENIED),
			// between BH and Coretime: denied
			(BridgeHubKusama, CoretimeKusama, TELEPORT_FEES, DENIED),
			// between People and Coretime: denied
			(PeopleKusama, CoretimeKusama, TELEPORT_FEES, DENIED),
			// between Penpal and Coretime: denied
			(PenpalA, CoretimeKusama, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
...
fn authorized_cross_chain_aliases() {
	...
	CoretimeKusama::fund_accounts(vec![(target.clone(), fees * 10)]);
	...
	CoretimeKusama::execute_with(|| {
		...
		assert_ok!(<CoretimeKusama as CoretimeKusamaPallet>::PolkadotXcm::add_authorized_alias(
			<CoretimeKusama as Chain>::RuntimeOrigin::signed(target.clone()),
	...
	test_cross_chain_alias!(
		vec![
			...
			(PenpalA, CoretimeKusama, RESERVE_TRANSFER_FEES, ALLOWED)
		],
		origin,
		target,
		fees
	...
	CoretimeKusama::execute_with(|| {
		...
		assert_ok!(
			<CoretimeKusama as CoretimeKusamaPallet>::PolkadotXcm::remove_all_authorized_aliases(
				<CoretimeKusama as Chain>::RuntimeOrigin::signed(target.clone())
			)
		);
	});
	...
	test_cross_chain_alias!(
		vec![(PenpalA, CoretimeKusama, RESERVE_TRANSFER_FEES, DENIED)],
		origin,
		target,
		fees
	);
