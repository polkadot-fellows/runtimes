...
fn relay_origin_assertions(t: RelayToSystemParaTest) {
	...
	Kusama::assert_xcm_pallet_attempted_complete(Some(Weight::from_parts(627_959_000, 7_200)));
...
fn limited_teleport_native_assets_from_relay_to_system_para_works() {
	...
	let dest = Kusama::child_location_of(PeopleKusama::para_id());
	let beneficiary_id = PeopleKusamaReceiver::get();
	let test_args = TestContext {
		sender: KusamaSender::get(),
		receiver: PeopleKusamaReceiver::get(),
	...
	test.set_assertion::<PeopleKusama>(para_dest_assertions);
