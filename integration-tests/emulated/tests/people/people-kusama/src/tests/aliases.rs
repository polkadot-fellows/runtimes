...
fn account_on_sibling_syschain_aliases_into_same_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubKusama, PeopleKusama, TELEPORT_FEES, ALLOWED),
			// between BH and People: allowed
			(BridgeHubKusama, PeopleKusama, TELEPORT_FEES, ALLOWED),
			// between Coretime and People: allowed
			(CoretimeKusama, PeopleKusama, TELEPORT_FEES, ALLOWED),
			// between Penpal and People: denied
			(PenpalA, PeopleKusama, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
		target,
		fees
...
fn account_on_sibling_syschain_cannot_alias_into_different_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubKusama, PeopleKusama, TELEPORT_FEES, DENIED),
			// between BH and People: denied
			(BridgeHubKusama, PeopleKusama, TELEPORT_FEES, DENIED),
			// between Coretime and People: denied
			(CoretimeKusama, PeopleKusama, TELEPORT_FEES, DENIED),
			// between Penpal and People: denied
			(PenpalA, PeopleKusama, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
...
fn authorized_cross_chain_aliases() {
	...
	PeopleKusama::fund_accounts(vec![(target.clone(), fees * 10)]);
	...
	PeopleKusama::execute_with(|| {
		...
		assert_ok!(<PeopleKusama as PeopleKusamaPallet>::PolkadotXcm::add_authorized_alias(
			<PeopleKusama as Chain>::RuntimeOrigin::signed(target.clone()),
	...
	test_cross_chain_alias!(
		vec![
			...
			(PenpalA, PeopleKusama, RESERVE_TRANSFER_FEES, ALLOWED)
		],
		origin,
		target,
		fees
	...
	PeopleKusama::execute_with(|| {
		...
		assert_ok!(
			<PeopleKusama as PeopleKusamaPallet>::PolkadotXcm::remove_all_authorized_aliases(
				<PeopleKusama as Chain>::RuntimeOrigin::signed(target.clone())
			)
		);
	});
	...
	test_cross_chain_alias!(
		vec![(PenpalA, PeopleKusama, RESERVE_TRANSFER_FEES, DENIED)],
		origin,
		target,
		fees
	);
