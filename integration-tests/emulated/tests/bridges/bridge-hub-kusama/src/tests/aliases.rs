...
use crate::*;
use bridge_hub_kusama_runtime::xcm_config::XcmConfig;
...
fn account_on_sibling_chain_cannot_alias_into_same_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubKusama, BridgeHubKusama, TELEPORT_FEES, DENIED),
			// between Penpal and BH: denied
			(PenpalA, BridgeHubKusama, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
		target,
		fees
...
fn authorized_cross_chain_aliases() {
	...
	BridgeHubKusama::fund_accounts(vec![(target.clone(), fees * 10)]);
	...
	BridgeHubKusama::execute_with(|| {
		let penpal_origin = Location::new(
			...
			X2([
				...
				AccountId32 { network: Some(Kusama), id: origin.clone().into() },
			]
			.into()),
		);
		// `target` adds `penpal_origin` as authorized alias
		assert_ok!(<BridgeHubKusama as BridgeHubKusamaPallet>::PolkadotXcm::add_authorized_alias(
			<BridgeHubKusama as Chain>::RuntimeOrigin::signed(target.clone()),
	...
	test_cross_chain_alias!(
		vec![
			...
			(PenpalA, BridgeHubKusama, RESERVE_TRANSFER_FEES, ALLOWED)
		],
		origin,
		target,
		fees
	...
	BridgeHubKusama::execute_with(|| {
		...
		assert_ok!(
			<BridgeHubKusama as BridgeHubKusamaPallet>::PolkadotXcm::remove_all_authorized_aliases(
				<BridgeHubKusama as Chain>::RuntimeOrigin::signed(target.clone())
			)
		);
	});
	...
	test_cross_chain_alias!(
		vec![(PenpalA, BridgeHubKusama, RESERVE_TRANSFER_FEES, DENIED)],
		origin,
		target,
		fees
	);
