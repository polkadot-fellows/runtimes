...
fn register_polkadot_asset_on_kah_from_pah() {
	...
	let bridged_asset_at_kah = v5::Location::new(
		2,
		[
			v5::Junction::GlobalConsensus(v5::NetworkId::Polkadot),
			v5::Junction::Parachain(AssetHubPolkadot::para_id().into()),
...
fn register_ethereum_asset_on_kah_from_pah() {
	...
	let bridged_asset_at_kah = v5::Location::new(
		2,
		[
			v5::Junction::GlobalConsensus(v5::NetworkId::Ethereum { chain_id: CHAIN_ID }),
			v5::Junction::AccountKey20 { network: None, key: WETH },
		],
	);
	// Register above asset on Kusama AH from Polkadot AH.
	register_asset_on_kah_from_pah(bridged_asset_at_kah);
...
fn register_asset_on_kah_from_pah(bridged_asset_at_kah: v5::Location) {
