...
fn account_on_sibling_syschain_aliases_into_same_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubPolkadot, CollectivesPolkadot, TELEPORT_FEES, ALLOWED),
			// between BH and Collectives: allowed
			(BridgeHubPolkadot, CollectivesPolkadot, TELEPORT_FEES, ALLOWED),
			// between Coretime and Collectives: allowed
			(CoretimePolkadot, CollectivesPolkadot, TELEPORT_FEES, ALLOWED),
			// between People and Collectives: allowed
			(PeoplePolkadot, CollectivesPolkadot, TELEPORT_FEES, ALLOWED),
			// between Penpal and Collectives: denied
			(PenpalA, CollectivesPolkadot, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
		target,
		fees
...
fn account_on_sibling_chain_cannot_alias_into_different_local_account() {
	...
	test_cross_chain_alias!(
		vec![
			...
			(AssetHubPolkadot, CollectivesPolkadot, TELEPORT_FEES, DENIED),
			// between BH and Collectives: denied
			(BridgeHubPolkadot, CollectivesPolkadot, TELEPORT_FEES, DENIED),
			// between Coretime and Collectives: denied
			(CoretimePolkadot, CollectivesPolkadot, TELEPORT_FEES, DENIED),
			// between People and Collectives: denied
			(PeoplePolkadot, CollectivesPolkadot, TELEPORT_FEES, DENIED),
			// between Penpal and Collectives: denied
			(PenpalA, CollectivesPolkadot, RESERVE_TRANSFER_FEES, DENIED)
		],
		origin,
...
fn authorized_cross_chain_aliases() {
	...
	CollectivesPolkadot::fund_accounts(vec![(target.clone(), fees * 10)]);
	...
	CollectivesPolkadot::execute_with(|| {
		...
		assert_ok!(
			<CollectivesPolkadot as CollectivesPolkadotPallet>::PolkadotXcm::add_authorized_alias(
				<CollectivesPolkadot as Chain>::RuntimeOrigin::signed(target.clone()),
	...
	test_cross_chain_alias!(
		vec![
			...
			(PenpalA, CollectivesPolkadot, RESERVE_TRANSFER_FEES, ALLOWED)
		],
		origin,
		target,
		fees
	...
	CollectivesPolkadot::execute_with(|| {
		...
		assert_ok!(
			<CollectivesPolkadot as CollectivesPolkadotPallet>::PolkadotXcm::remove_all_authorized_aliases(
				<CollectivesPolkadot as Chain>::RuntimeOrigin::signed(target.clone())
			)
		);
	});
	...
	test_cross_chain_alias!(
		vec![(PenpalA, CollectivesPolkadot, RESERVE_TRANSFER_FEES, DENIED)],
		origin,
		target,
		fees
	);
