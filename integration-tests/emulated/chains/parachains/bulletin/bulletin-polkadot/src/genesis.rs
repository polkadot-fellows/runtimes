...
pub fn genesis() -> Storage {
	let genesis_config = bulletin_polkadot_runtime::RuntimeGenesisConfig {
		...
		collator_selection: bulletin_polkadot_runtime::CollatorSelectionConfig {
			invulnerables: collators::invulnerables().iter().cloned().map(|(acc, _)| acc).collect(),
			candidacy_bond: ED * 16,
			..Default::default()
		},
		session: bulletin_polkadot_runtime::SessionConfig {
			keys: collators::invulnerables()
				...
				.map(|(acc, aura)| {
					(
						...
						acc,                                             // validator id
						bulletin_polkadot_runtime::SessionKeys { aura }, // session keys
					)
				})
				.collect(),
			..Default::default()
		},
		polkadot_xcm: bulletin_polkadot_runtime::PolkadotXcmConfig {
