...
use subxt::{ext::futures::StreamExt, OnlineClient};
...
async fn smoke() -> Result<(), anyhow::Error> {
	...
	let alice_client: OnlineClient<_> = alice.wait_client().await?;
	...
	let collator_client: OnlineClient<_> = collator.wait_client().await?;
