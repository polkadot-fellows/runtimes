...
async fn migration_works_time() {
	...
	let migrate = |ah_block_start: u32, rc: &mut TestExternalities, ah: &mut TestExternalities| {
			...
			AhMigrationStage::MigrationDone
		{
			...
			ah.execute_with(|| {
				next_block_ah_with_rc_block(rc_block - 1);
			});
...
async fn scheduled_migration_works() {
	...
	let (dmp_messages, cool_off_end_at) = rc.execute_with(|| {
		...
		// cool-off end
		assert_eq!(RcMigrationStageStorage::<Polkadot>::get(), RcMigrationStage::MigrationDone);
	...
	// Asset Hub receives the signal to finish the migration, enters the cool-off period and
	...
	ah.execute_with(|| {
		...
		next_block_ah_with_rc_block(cool_off_end_at - 1);

		assert_eq!(
			AhMigrationStageStorage::<AssetHub>::get(),
			AhMigrationStage::CoolOff { end_at: cool_off_end_at }
		);

		next_block_ah_with_rc_block(cool_off_end_at);

		assert_eq!(AhMigrationStageStorage::<AssetHub>::get(), AhMigrationStage::MigrationDone);
	});
}
...
async fn post_migration_checks_only() {
	...
	ah_post_ext.execute_with(|| {
		AhMigrationStageStorage::<AssetHub>::put(AhMigrationStage::MigrationDone);
